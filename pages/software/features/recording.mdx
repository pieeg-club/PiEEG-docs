...
For a browser take that pairs the live bio stream with 33-point camera pose (points only, two clocks, no pixels), see [BioPose Recorder](/software/features/biopose). Confirmed frontal blinks are stored as `blink` events on that take. That path does not write the server CSV above.

For a labelled Aura VR take (8-ch palm EMG + IMU, guided rest/hold, optional Hand Landmarker), see [Aura Dataset Lab](/software/features/aura-dataset). Native-rate JSON/ZIP; the 50 Hz table is labels only.
