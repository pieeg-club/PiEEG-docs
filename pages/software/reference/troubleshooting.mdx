...
The server binds two ports: **1616** (WebSocket stream) and **1617** (dashboard / HTTP). If either is taken — usually by a previous run or the systemd service — startup fails:
...
# or kill whatever is holding each port
kill $(lsof -t -i:1616)
kill $(lsof -t -i:1617)
```

<Callout type="info">
  **Need to start fresh?** See the complete [Uninstall & Purge guide](/software/getting-started/uninstall) for systemd service removal and clean reinstall instructions.
</Callout>
