...
{"cmd": "osc_stop"}
{"cmd": "osc_status"}
{"cmd": "osc_config", "config": {"host": "127.0.0.1", "port": 9000, "mode": "both", "interval": 0.25}}
```
...
{"cmd": "lsl_status"}
{"cmd": "lsl_groups_get"}
{"cmd": "lsl_groups_set", "groups": [{"name": "EEG", "channels": [0, 1, 2, 3, 4, 5, 6]}, {"name": "EOG", "channels": [7]}]}
```
...
Real-time register configuration for PiEEG boards. See [Register Configuration](/software/features/registers) for full details.
...
The `channels` field reflects the selected device: **8** for `pieeg8`/`ironbci8`, **16** for `pieeg16` (default), **32** for `ironbci32`.
